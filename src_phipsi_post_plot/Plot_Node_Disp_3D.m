...
global Model_Surface_Nodes DISP_Cylinder_CS
...
if Key_PLOT(4,10)==1 
	Plot_DISP = DISP ;    
	if isempty(DISP)
		disp('    > Can not find nodal disp result!') 
		return
	end
	String_01 = 'Displacement x contour';
	String_02 = 'Displacement y contour';
	String_03 = 'Displacement z contour';
	String_04 = 'Displacement vector sum contour';	
%柱坐标系结果
elseif Key_PLOT(4,10)==2
	Plot_DISP = DISP_Cylinder_CS ;   
	if isempty(DISP_Cylinder_CS)
		disp('    > Can not find nodal disp result (in Cylinderal CS)!') 
		return
	end
	String_01 = 'Displacement r contour';
	String_02 = 'Displacement θ contour';
	String_03 = 'Displacement z contour';	
	String_04 = 'Displacement vector sum contour';	
end

% Get the new coordinates of all nodes.
New_Node_Coor(1:Num_Node,1) = Node_Coor(1:Num_Node,1) + scale*Plot_DISP(1:Num_Node,2);
New_Node_Coor(1:Num_Node,2) = Node_Coor(1:Num_Node,2) + scale*Plot_DISP(1:Num_Node,3);
New_Node_Coor(1:Num_Node,3) = Node_Coor(1:Num_Node,3) + scale*Plot_DISP(1:Num_Node,4);
...
for i_Plot = Start_Dis_Type:End_Dis_Type
	...
	elseif i_Plot == 3
        ...
        cellfun(@patch,repmat({'Vertices'},1,num_plot_el),XYZ,.......
            ...
            repmat({'FaceColor'},1,num_plot_el),repmat({'interp'},1,num_plot_el));	
		set(gca,'XTick',[]) ; set(gca,'YTick',[]); set(gca,'ZTick',[]) ;			
	elseif i_Plot == 4  % 2022-05-29. NEWFTU-2022052901.
	    disp(['    > Plotting ',String_04]) 
        ...
        for iElem=1:num_plot_el	
            ...
            Z = New_Node_Coor(nd,3);
            XYZ{iElem} = [X Y Z] ;
            profile{iElem} = sqrt(Plot_DISP(nd,2).^2+Plot_DISP(nd,3).^2+Plot_DISP(nd,4).^2);
        end
        cellfun(@patch,repmat({'Vertices'},1,num_plot_el),XYZ,.......
            repmat({'Faces'},1,num_plot_el),repmat({fm},1,num_plot_el),......
            repmat({'FaceVertexCdata'},1,num_plot_el),profile,......
	...
	if i_Plot == 1
        clim(1)=min(Plot_DISP(:,2));
        clim(2)=max(Plot_DISP(:,2));	
	elseif i_Plot == 2
        clim(1)=min(Plot_DISP(:,3));
        clim(2)=max(Plot_DISP(:,3));	
	elseif i_Plot == 3
        clim(1)=min(Plot_DISP(:,4));
        clim(2)=max(Plot_DISP(:,4));			
	elseif i_Plot == 4
        clim(1)=min(sqrt(Plot_DISP(:,2).^2+Plot_DISP(:,3).^2+Plot_DISP(:,4).^2));
        clim(2)=max(sqrt(Plot_DISP(:,2).^2+Plot_DISP(:,3).^2+Plot_DISP(:,4).^2));					
	end
	...
	elseif i_Plot ==4
	    Save_Picture(c_figure,Full_Pathname,'Dpsn')			
	end	
