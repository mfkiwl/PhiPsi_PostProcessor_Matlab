...
for i = 1:1
	...
	if Key_PLOT(5,1)==1 | Key_PLOT(5,1)==3
		...
		clim(1)=min(Field_Value(1:Num_Node));
		clim(2)=max(Field_Value(1:Num_Node));
		if (abs(clim(1)-clim(2))<=1.0e-6) 
		    clim(2) = clim(1) + 1.0e-6;
		end
	...
	%提示. 2023-01-27. 输出的云图为空白，云图空白，则有可能是Outline和Inline检测错误导致的. KNISUE-2023012301.
	disp(['            提示：若输出的云图为空白，则注释掉以上两段代码!'])
