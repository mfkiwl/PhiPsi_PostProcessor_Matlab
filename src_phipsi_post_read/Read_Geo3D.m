...
%普通情形.
if size(Elem_Info,2) >=9
	Elem_Material = Elem_Info(:,9);       % Material number of elements
	num_of_Material = max(Elem_Material); % Number of materials
%用于新奥Key_Cpp_Call_Fortran_Lib=1的情形.
else
    Elem_Material(1:size(Elem_Info,1)) = 1;       % Material number of elements
	num_of_Material = 1; % Number of materials
end
...
if exist([Full_Pathname,'.fraz'], 'file') ==2
	...
	frac_zone_min_z = ttt_DATA(5); 
	frac_zone_max_z = ttt_DATA(6); 	
end
